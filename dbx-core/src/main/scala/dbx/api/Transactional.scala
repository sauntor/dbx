...
import org.springframework.transaction.interceptor.{DelegatingTransactionAttribute, TransactionAttribute, _}
import org.springframework.transaction.support.{CallbackPreferringPlatformTransactionManager, TransactionCallback}
import org.springframework.transaction.{NoTransactionException, PlatformTransactionManager, TransactionStatus, TransactionSystemException}
import org.springframework.util.{ConcurrentReferenceHashMap, StringUtils}

/**
  * Base class for transactional functions, such as the [[ProvidedDataSourceTransactional]].
  *
...
trait Transactional[R] {
  ...
  val settingsProvider: TransactionSettingsProvider
  private lazy val defaultSettings = settingsProvider.get("default")
  private [api] def exceptionTranslator(resource: String): Option[SQLErrorCodeSQLExceptionTranslator]

  def apply[T](resource: String = "default",
               readOnly: Option[Boolean] = None, isolation: Option[Isolation] = None,
               propagation: Option[Propagation] = None, timeout: Option[Int] = None,
               noRollbackFor: Option[Seq[Class[_]]] = None,
               rollbackFor: Option[Seq[Class[_]]] = None
              )(transactionalOperation: Resource => T): T = {

    val attribute = new RuleBasedTransactionAttribute
    attribute.setPropagationBehavior(propagation.getOrElse(settingsProvider.get(resource).propagation).id)
    attribute.setIsolationLevel(isolation.getOrElse(settingsProvider.get(resource).isolation).id)
    attribute.setTimeout(timeout.getOrElse(settingsProvider.get(resource).timeout))
    attribute.setReadOnly(readOnly.getOrElse(settingsProvider.get(resource).readOnly))
    attribute.setQualifier(resource)
    val rollBackRules = new java.util.ArrayList[RollbackRuleAttribute]
    rollbackFor foreach { classes =>
      for (klass <- classes) {
        rollBackRules.add(new RollbackRuleAttribute(klass))
      }
    }
    noRollbackFor foreach { classes =>
      for (klass <- classes) {
        rollBackRules.add(new NoRollbackRuleAttribute(klass))
      }
    }
    ...
    if (txAttr == null || !tm.isInstanceOf[CallbackPreferringPlatformTransactionManager]) {
      ...
      try
        ...
        case ex: Throwable => {
          ...
          throw translateExceptionIfNeeded(txAttr.getQualifier, ex)
        }
      ...
      try {
        ...
        if (result.isInstanceOf[ThrowableHolder]) throw translateExceptionIfNeeded(txAttr.getQualifier, result.asInstanceOf[ThrowableHolder].getThrowable)
        else return result.asInstanceOf[T]
      } catch {
        case ex: ThrowableHolderException => {
          throw translateExceptionIfNeeded(txAttr.getQualifier, ex.getCause)
        }
      }
    }
  }

  private def translateExceptionIfNeeded(resource: String, exception: Throwable) = {
    val translator = exceptionTranslator(resource)
    if (exception.isInstanceOf[SQLException] && translator.isDefined) {
      translator.get.translate("Transactional()", null, exception.asInstanceOf[SQLException])
    }
    else exception
  }
